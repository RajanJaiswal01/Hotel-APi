        ...
        
    {
        ...
        public async Task FilterUser()
        {
            ...
            foreach (var item in customer)
            {
                ...
                {
                    var invoice = _invoiceRepository.GetAll(x => x.CustomerId == item.CustomerId);
                    var booking = _bookingRepository.GetAll(x => x.CustomerId == item.CustomerId);
                    var customerr = _customerRepository.FindBy(x => x.CustomerId == item.CustomerId);
        ...
        {
            Customer customer = await _customerRepository.GetSingle(model.CustomerId);
            if (customer != null)
            {
                customer.CustomerId = model.CustomerId;
                customer.Age = model.Age;
